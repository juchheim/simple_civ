...
echo "Starting simulation (1 sim per map size)..."
cd /Users/ejuchheim/Projects/Simple-Civ/SimpleCiv
SIM_SEEDS_COUNT=1 node engine/dist/sim/comprehensive-analysis.js > "$LOG_FILE" 2>&1 &
SIM_PID=$!
...
while kill -0 "$SIM_PID" 2>/dev/null; do
    ...
    # Grep for /5 because we are running 5 total sims (1 per map size * 5 map sizes)
    CURRENT=$(grep "\[[0-9]*/5\]" "$LOG_FILE" 2>/dev/null | tail -1 | grep -o "\[[0-9]*/5\]" | grep -o "[0-9]*" | head -1 || echo "0")
    COMPLETED=$(grep -c "✓ Completed in" "$LOG_FILE" 2>/dev/null || echo "0")
    
    if [ "$CURRENT" != "0" ]; then
        PERCENT=$((CURRENT * 100 / 5))
        echo "[$(date +%H:%M:%S)] Status: $CURRENT/5 simulations ($PERCENT%) - $COMPLETED completed"
    else
