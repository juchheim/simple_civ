...
const TARGET_CIV = "ScholarKingdoms";
...
results.forEach(sim => {
    const scholarCiv = sim.finalState?.civs.find(c => c.civName === TARGET_CIV);
    if (!scholarCiv) return; // Scholar Kingdoms didn't participate in this game

    const scholarId = scholarCiv.id;

    // Find elimination event
    const eliminationEvent = sim.events.find(e =>
        e.type === "Elimination" && e.eliminated === scholarId
    );

    // Track all events involving Scholar Kingdoms
    const warsReceived = sim.events.filter(e =>
        e.type === "WarDeclaration" && e.target === scholarId
    );
    const warsInitiated = sim.events.filter(e =>
        e.type === "WarDeclaration" && e.initiator === scholarId
    );
    const citiesLost = sim.events.filter(e =>
        e.type === "CityCapture" && e.from === scholarId
    );
    const citiesCaptured = sim.events.filter(e =>
        e.type === "CityCapture" && e.to === scholarId
    );
    const citiesFounded = sim.events.filter(e =>
        e.type === "CityFound" && e.owner === scholarId
    );
    const techsResearched = sim.events.filter(e =>
        e.type === "TechComplete" && e.civ === scholarId
    );
    const projectsCompleted = sim.events.filter(e =>
        e.type === "ProjectComplete" && e.civ === scholarId
    );
    const buildingsBuilt = sim.events.filter(e =>
        e.type === "BuildingComplete" && e.owner === scholarId
    );
    const unitsDied = sim.events.filter(e =>
        e.type === "UnitDeath" && e.owner === scholarId
    );
    const unitsProduced = sim.events.filter(e =>
        e.type === "UnitProduction" && e.owner === scholarId
    );
    ...
    const gameData = {
        ...
        isWinner: sim.winner?.id === scholarId,
        victoryType: sim.winner?.id === scholarId ? sim.victoryType : null,
        isEliminated: scholarCiv.isEliminated || !!eliminationEvent,
        eliminationTurn: eliminationEvent?.turn || null,
        eliminatedBy: eliminationEvent?.by ? getCivName(sim, eliminationEvent.by) : null,

        // Final state
        finalCities: scholarCiv.cities,
        finalPop: scholarCiv.totalPop,
        finalTechs: scholarCiv.techs,
        finalProjects: scholarCiv.projects,
        finalPower: scholarCiv.militaryPower,
    ...
    scholarData.push(gameData);
});

console.log(`Scholar Kingdoms participated in ${scholarData.length} games\n`);

// ============================================================================
// GENERATE REPORT
// ============================================================================

let report = `# Scholar Kingdoms Deep Dive Analysis\n\n`;
report += `**Date:** ${new Date().toISOString().split('T')[0]}\n`;
report += `**Games Analyzed:** ${scholarData.length}\n\n`;

// Overall Stats
const wins = scholarData.filter(d => d.isWinner);
const eliminations = scholarData.filter(d => d.isEliminated);
const survived = scholarData.filter(d => !d.isWinner && !d.isEliminated);

report += `## Summary\n\n`;
report += `| Metric | Value |\n`;
report += `|--------|-------|\n`;
report += `| Games Played | ${scholarData.length} |\n`;
report += `| **Wins** | ${wins.length} (${((wins.length / scholarData.length) * 100).toFixed(1)}%) |\n`;
report += `| **Eliminations** | ${eliminations.length} (${((eliminations.length / scholarData.length) * 100).toFixed(1)}%) |\n`;
...
report += `> [!IMPORTANT]\n`;
report += `> This section tracks exactly when Scholar Kingdoms was eliminated and by whom.\n\n`;
...
// Wars received
const allWarsReceived = scholarData.flatMap(d => d.warsReceived.map(w => ({ ...w, seed: d.seed, mapSize: d.mapSize })));
...
report += `- **Average Wars per Game:** ${(allWarsReceived.length / scholarData.length).toFixed(1)}\n\n`;
...
const allCitiesLost = scholarData.flatMap(d => d.citiesLost.map(c => ({ ...c, seed: d.seed })));
...
report += `- **Average Cities Lost per Game:** ${(allCitiesLost.length / scholarData.length).toFixed(1)}\n\n`;
...
const avgFinalTechs = scholarData.reduce((s, d) => s + d.finalTechs, 0) / scholarData.length;
const avgFinalProjects = scholarData.reduce((s, d) => s + d.finalProjects, 0) / scholarData.length;
...
const observatoryCount = scholarData.filter(d => d.projectsCompleted.some(p => p.project === "Observatory")).length;
const academyCount = scholarData.filter(d => d.projectsCompleted.some(p => p.project === "GrandAcademy")).length;
const experimentCount = scholarData.filter(d => d.projectsCompleted.some(p => p.project === "GrandExperiment")).length;

report += `**Progress Chain Completion:**\n`;
report += `- Observatory: ${observatoryCount} games (${((observatoryCount / scholarData.length) * 100).toFixed(1)}%)\n`;
report += `- Grand Academy: ${academyCount} games (${((academyCount / scholarData.length) * 100).toFixed(1)}%)\n`;
report += `- Grand Experiment: ${experimentCount} games (${((experimentCount / scholarData.length) * 100).toFixed(1)}%)\n\n`;
...
const allUnitsDied = scholarData.flatMap(d => d.unitsDied);
const allUnitsProduced = scholarData.flatMap(d => d.unitsProduced);
...
const allBuildings = scholarData.flatMap(d => d.buildingsBuilt);
...
if (eliminations.length > 0) {
    ...
    report += `- **Most Dangerous Opponent:** ${topEliminator[0]} (eliminated Scholar Kingdoms ${topEliminator[1]} times)\n`;
...
writeFileSync('/tmp/scholar-kingdoms-analysis.md', report);
console.log("Scholar Kingdoms analysis complete!");
console.log(`Report written to /tmp/scholar-kingdoms-analysis.md`);
