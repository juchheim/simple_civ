...
function handleAttack(state: GameState, action: { type: "Attack"; playerId: string; attackerId: string; targetId: string; targetType: "Unit" | "City" }): GameState {
    ...
    if (attacker.movesLeft <= 0) throw new Error("No moves left to attack"); // Usually attacking takes a move or just ends turn?
    // Rulebook 2.4.1: "Move up to Move value. If eligible, attack once."
    // Does attacking consume movement? Usually yes or it's a separate action.
    // "If eligible, attack once."
    // Let's assume it doesn't cost movement points but requires the unit to be able to act?
    // Or maybe it ends movement?
    // Let's assume it's free but sets `hasAttacked = true`.

    const attackerStats = getEffectiveUnitStats(attacker, state);

    // Auto-declare war if currently at peace
    ...
    if (action.targetType === "Unit") {
        ...
        // For pure function, we should use state.seed and update it.
        // Let's implement a simple deterministic random based on state.seed.
        ...
        if (city.hp <= 0) {
            if (attackerStats.canCaptureCity && dist === 1) {
                // Move into city
                // Check movement? "An enemy Melee or Cavalry unit enters the tile using movement"
                // So they must have movement left to enter.
                // We are in Attack action.
                // If we killed it, we can try to move in?
                // Or does Attack auto-move on kill?
                // Rulebook 2.6.5.4: "Capturing Cities ... An enemy Melee or Cavalry unit enters the tile using movement (may happen after the reducing attack)."
                // So it's a separate Move action after reducing HP to <= 0.
                // So we don't capture here. We just reduce HP.
                // But wait, if HP <= 0, city is "Capturable".
                // We need to allow MoveUnit to enter city tile if HP <= 0.
            }
        }
    }
...
function handleFoundCity(state: GameState, action: { type: "FoundCity"; playerId: string; unitId: string; name: string }): GameState {
    ...
    if (unit.ownerId !== action.playerId) throw new Error("Not your unit");
    if (unit.movesLeft <= 0) throw new Error("No moves left"); // Founding takes action?
    ...
    // Territory check: no shared tiles within work radius
    ...
    // Claim territory (rings 0-2)
    claimCityTerritory(newCity, state, action.playerId);

    // Initialize worked tiles to best available up to Pop
...
function handleSetCityBuild(state: GameState, action: { type: "SetCityBuild"; playerId: string; cityId: string; buildType: "Unit" | "Building" | "Project"; buildId: string }): GameState {
    ...
    // Consumes population if Settler?
    // "When a city produces a Settler... That city loses 1 Pop".
    // This happens on completion, not start.
...
function handleRazeCity(state: GameState, action: { type: "RazeCity"; playerId: string; cityId: string }): GameState {
    ...
    // Require a friendly unit on the city tile to raze
...
function handleSetDiplomacy(state: GameState, action: { type: "SetDiplomacy"; playerId: string; targetPlayerId: string; state: DiplomacyState }) {
...
function handleProposePeace(state: GameState, action: { type: "ProposePeace"; playerId: string; targetPlayerId: string }) {
    const a = action.playerId;
    const b = action.targetPlayerId;
    if (!state.players.find(p => p.id === b)) throw new Error("Target player not found");
    assertContact(state, a, b);
...
function handleAcceptPeace(state: GameState, action: { type: "AcceptPeace"; playerId: string; targetPlayerId: string }) {
    const a = action.playerId;
    const b = action.targetPlayerId;
    assertContact(state, a, b);
...
function handleProposeVisionShare(state: GameState, action: { type: "ProposeVisionShare"; playerId: string; targetPlayerId: string }) {
    const a = action.playerId;
    const b = action.targetPlayerId;
...
function handleAcceptVisionShare(state: GameState, action: { type: "AcceptVisionShare"; playerId: string; targetPlayerId: string }) {
    const a = action.playerId;
    const b = action.targetPlayerId;
    assertCanShareVision(state, a, b);
...
function handleRevokeVisionShare(state: GameState, action: { type: "RevokeVisionShare"; playerId: string; targetPlayerId: string }) {
    const a = action.playerId;
    const b = action.targetPlayerId;
    disableSharedVision(state, a, b);
    return state;
...
function handleEndTurn(state: GameState, action: { type: "EndTurn"; playerId: string }): GameState {
    // Check if it's actually this player's turn (already checked in applyAction)
    ...
    // If we wrapped around to first player, increment turn and run global round logic?
    // "A global round consists of each player taking a full turn in order."
    // "End of Global Round: After all players have taken a turn."
    if (nextPIdx === 0) {
        state.turn += 1;
        // End of global round resolution
        runEndOfRound(state);
    }

    // Run Start of Turn for the new player
...
function advancePlayerTurn(state: GameState, playerId: string): GameState {
    ...
    // 1. City Yields & Growth & Production
    for (const city of state.cities.filter(c => c.ownerId === playerId)) {
        // Make sure worked tiles align with current Pop/territory
        ...
        // Science (Global)
        // Accumulated per city, added to player total
...
function completeBuild(state: GameState, city: City) {
    ...
    if (build.type === "Unit") {
        ...
        state.units.push({
            ...
            coord: city.coord, // Spawn at city center
            hp: UNITS[uType].hp,
        ...
        // Handle Project effects (Victory, etc.)
        ...
        if (pId === ProjectId.GrandExperiment) {
            // Victory handled above; milestone tracked on player
        }
...
function healUnitsAtStart(state: GameState, playerId: string) {
    for (const unit of state.units.filter(u => u.ownerId === playerId)) {
        const stats = UNITS[unit.type];
        // If unit still has full movement remaining and didn't attack last turn, it rested
...
function checkConquestVictory(state: GameState): string | null {
    ...
    for (const p of alivePlayers) {
        ...
        // Must have at least one city to win
...
function ensureWorkedTiles(city: City, state: GameState): HexCoord[] {
    ...
    // Fill missing slots with best tiles
...
function buildTileLookup(state: GameState): Map<string, import("../core/types.js").Tile> {
    return new Map(state.map.tiles.map(t => [hexToString(t.coord), t]));
}

function hasClearLineOfSight(state: GameState, from: HexCoord, target: HexCoord, lookup?: Map<string, import("../core/types.js").Tile>): boolean {
