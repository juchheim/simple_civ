...
import type { UnitStats } from "./constants";
...
export function applyAction(state: GameState, action: Action): GameState {
    ...
    switch (action.type) {
        ...
        case "EndTurn":
            ...
            logStateSnapshot(`applyAction:EndTurn:after (${action.playerId})`, updatedState);
            break;
        default:
...
function enforceLinkedUnitIntegrity(state: GameState) {
    ...
    state.units.forEach(u => lookup.set(u.id, u));

    state.units.forEach(unit => {
        if (!unit.linkedUnitId) return;
        const partner = lookup.get(unit.linkedUnitId);
