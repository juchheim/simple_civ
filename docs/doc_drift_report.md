...
- ✅ City territory and tile working: cities now claim radius-2 territory on founding (no-sharing enforced) and auto-assign worked tiles up to Pop, re-evaluated on growth and population loss (`engine/src/game/turn-loop.ts`, `client/src/utils/turn-loop.ts`).
- ✅ City capture/raze: MoveUnit now captures enemy cities at 0 HP (melee/cav only), resets HP/pop, transfers territory, and a Raze action clears the city/territory (`engine/src/game/turn-loop.ts`, client mirror).
