...
- **3D Board Planning (Jun 1, 2026):** Created planning + implementation doc set in `docs/3d-board/` (README + 00–09) for an optional 3D rendering of the board, on branch `feature/3d-board`.
  ...
  - **Key decisions/relationships:**
    ...
    - **Preference:** new `boardMode` (`"2d"` default) via localStorage hook mirroring `useCombatPreviewPreference.ts`, plumbed `useAppUiFlow → useAppCoreState → useAppPresenterModel → InGameContent → HUD → GameMenu` (like `showShroud`); shown in Game→Preferences (`HUD/sections/GameMenu.tsx`). WebGL capability gate + error-boundary fallback to 2D.
