...
- **Wonders**: not otherwise defined in v0.96 beyond unique buildings/projects above.

## 14. Civilizations & Traits
- **ForgeClans**: +1 Production from each worked Hill.
- **Scholar Kingdoms**: +1 Science in cities with Pop ≥5.
- **River League**: +1 Food from each worked river-adjacent tile.
- **Aetherian Vanguard**: Can build Titan's Core (summon Titan). **Battle Hardened**: Military units gain +1 HP per era researched (max +3).
- **Starborne Seekers**: Can build Spirit Observatory (Revelation bonus).
- **Jade Covenant**: Can build Jade Granary (Great Harvest bonus). **Bountiful Harvest**: Cities start with +5 stored Food when founded.
