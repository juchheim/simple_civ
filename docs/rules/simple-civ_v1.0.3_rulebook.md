...
- **Starting units**: each civ begins with 1 Settler + 1 Scout + 1 Spear Guard. **Starborne Seekers** gain an extra Scout. **Scholar Kingdoms** gain an extra Bow Guard.
...
- **Defense & HP**:
  ...
  - City Ward adds +3 defense and +1 city attack. Shield Generator adds +15 defense and 50 Shield HP (regenerates 5/turn).
- **City Attack**:
...
## 9. Units
- **Costs scale with time**: all unit costs use `Base × (1 + floor(Turn/25))`.
...
- **Army Units** (HP 15; enabled by Drilled Ranks + Army Doctrine):
...
- **Aether Era Units**:
  - Airship: 0/10/0/4/20/150, Air, no capture, vision 4 (support/scout).
  - Landship: 14/10/1/3/25/220, Land, can capture, vision 2 (late-game siege/capture).
- **Civ-exclusive Units**:
  - Lorekeeper (Scholar/Starborne only): 4/6/2/1/12/55, Land, no capture, vision 2; gains +3 Defense in friendly territory or on own city and +50% Defense vs Army units while defending in territory.
  - **Titan** (Aetherian Vanguard only): 22/6/1/2/25, capture, vision 2; regenerates 1 HP/turn in enemy/neutral, 2 in friendly territory, 4 in friendly city.
...
- **Unlocks** (one per tech):
  ...
  - **Aether Era**:
    ...
    - Zero Point Energy → Aether Reactor.
    - Composite Armor → Landship.
    - Plasma Shields → Shield Generator.
    - Dimensional Gate → Global Mobility (+1 Movement to all units).
...
- Costs are Production; one per city unless noted.
  ...
  - **City Ward** (40, City Wards): +3 city defense, +1 city attack.
  - **Forgeworks** (80, Steam Forges): +4 Production.
  - **City Square** (80, Urban Plans): +2 Food, +2 Production.
  - **Titan's Core** (220, Steam Forges, Aetherian only): Summons **The Titan** (limit 1 per civ).
  - **Spirit Observatory** (220, Star Charts, Starborne only): +5 Science, +4 Food; counts as the Observatory milestone (Progress chain). Grants +1 Science per city for Starborne when completed.
  ...
  - **Bulwark** (60, Stonework Halls, Scholar/Starborne only): +8 Defense, +3 City Attack, +1 Science. City cannot build Army units.
...
- **Progress Chain** (once per civ, one city at a time):
  - Observatory (250, Star Charts) → milestone (+1 Science in that city), unlocks Grand Academy.
  - Grand Academy (320, after Observatory) → +1 Science per city, unlocks Grand Experiment.
  - Grand Experiment (400, after Grand Academy) → Progress Victory on completion.
- **Form Army Projects** (Army Doctrine) [Costs scale with turn]:
  - Convert Spear Guard → Army Spear Guard.
  - Convert Bow Guard → Army Bow Guard.
  - Convert Riders → Army Riders.
  - Costs scale with turn using the same `Base × (1 + floor(Turn/25))` rule as units.
...
- **Wonders**: covered by unique buildings above (Titan's Core, Spirit Observatory, Jade Granary).
