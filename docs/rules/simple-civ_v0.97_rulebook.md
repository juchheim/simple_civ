...
## [DEV-ONLY] v0.97 Balance Changes
Major balance changes from v0.96 based on 50 AI simulation analysis:
- **Settler HP increased from 1 to 5**: Settlers were dying at 92.5% rate. Now survive at least one attack.
- **ScholarKingdoms nerf**: Changed from "+1 Science per city at Pop≥5" to "+2 Science in Capital only". Removes scaling snowball while maintaining strong early bonus.
- **JadeCovenant buff**: Added "Verdant Growth" passive — +10% faster growth globally (stacks with Farmstead/Jade Granary). Total growth reduction with all bonuses: 0.9 × 0.85 × 0.9 = 0.6885 (~31% faster growth).
...
- **Titan Rampage**: When AetherianVanguard spawns a Titan, they automatically switch to Conquest mode. The Titan aggressively seeks out and captures enemy cities, prioritizing capitals.
- **AI Escort Linking**: AI now links military escorts to settlers using the unit linking system. Linked pairs move together and can't be separated.
- **Proactive Settler Fleeing**: Settlers now flee from ANY nearby foreign military unit (not just war enemies) within 3 tiles.
<!-- CODEX_SKIP_END -->
...
- **Base Units** (atk/def/rng/move/HP/cost/domain/capture/vision):
  - Settler: 0/0/1/1/5/70, Civilian, cannot capture, vision 2. *(v0.97: HP increased from 1 to 5)*
...
- **Wonders**: not otherwise defined in v0.97 beyond unique buildings/projects above.

## 14. Civilizations & Traits
- **ForgeClans**: +1 Production from each worked Hill.
- **Scholar Kingdoms**: **Royal Academy** — +2 Science in Capital only. *(v0.97: Changed from "+1 Science per city at Pop≥5" to remove scaling snowball)*
- **River League**: +1 Food from each worked river-adjacent tile. **River Commerce**: +1 Production in cities on river tiles. *(v0.97: Added River Commerce passive)*
- **Aetherian Vanguard**: Can build Titan's Core (summon Titan). **Battle Hardened**: Military units gain +1 HP per era researched (max +3). **Titan's Wrath**: When a Titan is spawned, automatically switches to aggressive Conquest mode. *(v0.97: Added Titan's Wrath AI behavior)*
- **Starborne Seekers**: Can build Spirit Observatory (Revelation bonus).
- **Jade Covenant**: Can build Jade Granary (Great Harvest bonus). **Bountiful Harvest**: Cities start with +5 stored Food when founded. **Verdant Growth**: +10% faster growth globally (stacks with Farmstead/Jade Granary). *(v0.97: Added Verdant Growth passive)*
